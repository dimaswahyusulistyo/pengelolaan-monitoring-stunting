<template>
    <VRow>
      <VCol cols="12">
        <VCard title="ROLE MANAGEMENT" class="rounded-lg">
                ...
                <tr v-for="(role, index) in roles" :key="role.id_role">
                  ...
                  <td class="text-center">
                    <div class="button-group">
                      <VBtn icon color="warning" @click="editRole(role)" class="square-button">
                        <i class="bx bxs-edit-alt"></i>
                      </VBtn>
                      <VBtn icon color="error" @click="deleteRole(role)" class="square-button">
          ...
          <div class="d-flex align-center justify-space-between pa-4">
            <div class="d-flex align-center">
              ...
              <VSelect v-model="itemsPerPage" :items="[5, 10, 15, 20]" variant="outlined" density="compact"
                hide-details class="items-per-page-select" style="width: 80px;" @update:modelValue="fetchRoles" />
            </div>
  
            <VPagination v-model="currentPage" :length="totalPages" :total-visible="7" rounded="lg" @update:modelValue="fetchRoles" />
      ...
      <TambahRole v-model:dialog="dialog" @save="fetchRoles" />
...
import TambahRole from './tambah-role.vue';
...
const fetchRoles = async () => {
    try {
        console.log('Fetching Roles with per_page:', itemsPerPage.value);
...
const deleteRole = async (role) => {
    if (!role || !role.id_role) {
        console.error("Error: Role tidak valid!", role);
        return;
    }
    ...
    if (result.isConfirmed) {
        try {
            await apiClient.delete(`/roles/${role.id_role}`);
            fetchRoles();
